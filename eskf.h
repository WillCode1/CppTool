...
public:
    ...
    explicit ESKF(double acc_n = 1e-2, double gyr_n = 1e-4, double acc_w = 1e-6,
                double gyr_w = 1e-8)
        : acc_noise_(acc_n),
          gyr_noise_(gyr_n),
    ...
    void set_cov(double sigma_p, double sigma_v, double sigma_rp,
                 double sigma_yaw, double sigma_ba, double sigma_bg) {
        state_ptr_->cov.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity() *
                                            sigma_p *
                                            sigma_p;  // pos std: sigma_p m
        state_ptr_->cov.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() *
                                            sigma_v *
                                            sigma_v;  // vel std: sigma_v m/s
        state_ptr_->cov.block<2, 2>(6, 6) = Eigen::Matrix2d::Identity() *
                                            sigma_rp *
                                            sigma_rp;   // roll pitch std
        state_ptr_->cov(8, 8) = sigma_yaw * sigma_yaw;  // yaw std
        state_ptr_->cov.block<3, 3>(9, 9) =
            Eigen::Matrix3d::Identity() * sigma_ba * sigma_ba;  // Acc bias
        state_ptr_->cov.block<3, 3>(12, 12) =
            Eigen::Matrix3d::Identity() * sigma_bg * sigma_bg;  // Gyr bias
    }

    void imu_callback(const aw_idl::IMUConstPtr &imu_msg) {
        ImuDataPtr imu_data_ptr = std::make_shared<ImuData>();
        imu_data_ptr->timestamp = imu_msg->header.stamp.toSec();

        Eigen::Vector3d gyr(imu_msg->gyroscopes[0], imu_msg->gyroscopes[1],
                            imu_msg->gyroscopes[2]);
        Eigen::Vector3d acc(imu_msg->accelerometers[0],
                            imu_msg->accelerometers[1],
                            imu_msg->accelerometers[2]);


        //Eigen::Quaterniond ext_q(1.0, 0 /*x*/, 0 /*y*/, 0 /*z*/);
        Eigen::Quaterniond ext_q(0.0, 1 /*x*/, 0 /*y*/, 0 /*z*/);
        //Eigen::Quaterniond ext_q(0.0, -0.9971888 /*x*/, 0.0749297 /*y*/, 0 /*z*/);

        acc = ext_q * acc;
        gyr = ext_q * gyr;

        imu_data_ptr->acc[0] = acc(0);
        imu_data_ptr->acc[1] = acc(1);
        imu_data_ptr->acc[2] = acc(2);
        imu_data_ptr->gyr[0] = gyr(0);
        imu_data_ptr->gyr[1] = gyr(1);
        imu_data_ptr->gyr[2] = gyr(2);
    ...
    void predict(ImuDataConstPtr last_imu, ImuDataConstPtr curr_imu) {
        ...
        const Eigen::Vector3d acc_unbias =
            0.5 * (last_imu->acc + curr_imu->acc) - last_state.acc_bias;
        const Eigen::Vector3d gyr_unbias =
            0.5 * (last_imu->gyr + curr_imu->gyr) - last_state.gyr_bias;
        const Eigen::Vector3d acc_nominal =
            last_state.r_GI * acc_unbias + Eigen::Vector3d(0, 0, -kG);
        state_ptr_->p_GI =
            last_state.p_GI + last_state.v_GI * dt + 0.5 * acc_nominal * dt2;
        ...
        Fx.block<3, 3>(3, 6) =
            -state_ptr_->r_GI * aw::skew_matrix(acc_unbias) * dt;
        ...
        // Fi
        Eigen::Matrix<double, kStateDim, kNoiseDim> Fi =
            Eigen::Matrix<double, kStateDim, kNoiseDim>::Zero();
        Fi.block<12, kNoiseDim>(3, 0) =
            Eigen::Matrix<double, 12, kNoiseDim>::Identity();

        // Qi
        Eigen::Matrix<double, kNoiseDim, kNoiseDim> Qi =
            Eigen::Matrix<double, kNoiseDim, kNoiseDim>::Zero();
        Qi.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity() * dt2 * acc_noise_;
        Qi.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() * dt2 * gyr_noise_;
        Qi.block<3, 3>(6, 6) =
            Eigen::Matrix3d::Identity() * dt * acc_bias_noise_;
        Qi.block<3, 3>(9, 9) =
            Eigen::Matrix3d::Identity() * dt * gyr_bias_noise_;
    ...
    void update_K(const Eigen::MatrixBase<H_type> &H,
                  const Eigen::MatrixBase<R_type> &R,
                  Eigen::MatrixBase<K_type> &K) {
        const MatrixSD &P = state_ptr_->cov;
        const R_type &S = H * P * H.transpose() + R;
        K = P * H.transpose() * S.inverse();
        // const Eigen::Matrix<double, kStateDim, R_type::RowsAtCompileTime> K =
    ...
    void update_P(const Eigen::MatrixBase<H_type> &H,
        ...
        state_ptr_->cov =
            I_KH * state_ptr_->cov * I_KH.transpose() + K * R * K.transpose();
    }

    bool init_from_imudata(const Eigen::Matrix3d &rot,
                               const Eigen::Vector3d &p) {
        const std::deque<ImuDataConstPtr> imu_buf = imu_buf_;
        ...
        const Eigen::Vector3d std_acc =
            (sum_err2 / (double)imu_buf.size()).cwiseSqrt();
        ...
        state_ptr_->r_GI = r_IG.transpose();
        state_ptr_->p_GI = p;
    ...
    bool init_rot_from_imudata(Eigen::Matrix3d &r_GI,
                               const std::deque<ImuDataConstPtr> &imu_buf) {
        // mean and std of IMU accs
        Eigen::Vector3d sum_acc(0., 0., 0.);
        for (const auto imu_data : imu_buf) {
            sum_acc += imu_data->acc;
