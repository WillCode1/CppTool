...
public:
  ...
  EdgeSE2SemanticProject(cv::Mat *image, Eigen::Vector3d wp, double cam_height,
                         double baselink2cam)
  ...
  EdgeSE2SemanticProject()
      : image_(nullptr), cam_height_(4.0), baselink2cam_(1.3),
        coef_(1.0 / 255) {}
  ...
  virtual void computeError() {
    ...
    Vector3D point_camera(-point_vehicle.y(),
                          -point_vehicle.x() + baselink2cam_, cam_height_);
    Vector2D uv = cam_->cam_map(point_camera);

    if (uv.x() - 4 < 0 || (uv.x() + 4) > image_->cols || (uv.y() - 4) < 0 ||
        (uv.y() + 4) > image_->rows) {
      _error(0, 0) = 0.0;
      this->setLevel(1);
    } else {
