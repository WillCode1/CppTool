...
HpaMap::HpaMap() : map_changed_(false) {
  ...
  quadtree_ = std::unique_ptr<quadtree::Quadtree<SemanticSLAM::Node *>>(
      new quadtree::Quadtree<SemanticSLAM::Node *>(map_region));
}
...
void HpaMap::SaveMap(const std::string &filename) {
  ...
  std::lock_guard<std::mutex> lock2(mutex_map_);
  if (!map_changed_) {
    std::cout << kColorRed << " map has already saved " << kColorReset
              << std::endl;
    return;
  }
...
std::vector<PointTyped> HpaMap::GetSemanticPoints() {
  std::lock_guard<std::mutex> lock(mutex_map_);
  const std::vector<quadtree::SemanticBox> boxes =
      quadtree_->GetIncrementalBoxes();
  ...
  for (auto &box : boxes) {
    ...
    p.point =
        Vec3_t(origin.x() + 0.5 * size.x(), origin.y() + 0.5 * size.y(), 0);
    p.type = box.label_;
