...
AVPPose AvpMapperSystem::GrabSegImages(uint64 microsecond,
  ...
  if (!ret) {
    std::cout << kColorRed << " Can't Find Correspond Odometry " << kColorReset
              << std::endl;
    return current_pose;
  }
...
std::vector<std::pair<cv::Mat, cv::Mat>> AvpMapperSystem::GetPanoramicRemap() {
  ...
  if (remaps_.size() != 4) {
    ...
    std::cout << kColorRed << " REMAP SIZE IS WRONG " << kColorReset
              << std::endl;
  }
  return remaps_;
}
...
void AvpMapperSystem::LoadRemap(const std::string &config) {
  ...
  if (node_remap.empty()) {
    ...
    std::cout << kColorRed << "CAN NOT FIND REMAP IN CONFIG" << kColorReset
              << std::endl;
    return;
  }
  ...
  if (!fs_remap.isOpened()) {
    ...
    std::cout << kColorRed << "REMAP FILE DOES NOT EXIST" << kColorReset
              << std::endl;
    return;
  }
  remaps_.push_back(
      std::make_pair(fs_remap["mapx_left"].mat(), fs_remap["mapy_left"].mat()));
  remaps_.push_back(std::make_pair(fs_remap["mapx_front"].mat(),
                                   fs_remap["mapy_front"].mat()));
  remaps_.push_back(std::make_pair(fs_remap["mapx_right"].mat(),
                                   fs_remap["mapy_right"].mat()));
  remaps_.push_back(
      std::make_pair(fs_remap["mapx_back"].mat(), fs_remap["mapy_back"].mat()));
  fs_remap.release();
}

std::shared_ptr<AvpMapper> CreateAvpMapper(const std::string &config_file) {
  std::shared_ptr<AvpMapper> shared_mapper_ptr =
      std::make_shared<AvpMapperSystem>(config_file);
