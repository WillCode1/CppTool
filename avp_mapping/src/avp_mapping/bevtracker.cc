...
BevTracker::BevTracker(const std::string &config,
                       const std::shared_ptr<HpaMap> &hpa_map)
    : state_(TrackingState::NOT_INITIALIZED), map_(hpa_map),
      trajectory_length_(0), ref_keyfm_(nullptr) {
  ...
  avp_distance_transformer_ =
      std::make_shared<AvpDistanceTransformer>(image_height, image_width);
#ifdef ENABLE_VIEWER
  std::shared_ptr<std::thread> viewer_thread = std::make_shared<std::thread>(
      &HpaViewer::Run,
      &HpaViewer::GetInstance(image_resolution.first, image_resolution.second));
  ...
  std::shared_ptr<std::thread> plot_viewer_thread =
      std::make_shared<std::thread>(&PlotViewer::Run,
                                    &PlotViewer::GetInstance());
  ...
  // track log
  odometry_log_ =
      std::make_shared<TrajectoryLog>("", TrajectoryLog::Format::NULLMAX);
  trajectory_log_ =
      std::make_shared<TrajectoryLog>("", TrajectoryLog::Format::NULLMAX);
}

bool BevTracker::GrabSegImages(uint64_t microsecond,
                               std::vector<unsigned char *> seg_imgs,
                               WheelOdometry odom, AVPPose &vehicle_pose) {
  // generate energy mat
  ...
  trajectory_log_->ExportPose(
      microsecond, Utils::Matrix3d2Matrix4d(current_frame_.trans_world2base_));
...
void BevTracker::Track() {
  ...
  if (state_ == TrackingState::NOT_INITIALIZED) {
    ...
    if (SystemConfig::GetSystemConfig()->use_vision_opt_) {
      Optimizer::GetInstance().OptimizeFramePose(ref_keyfm_, current_frame_,
                                                 camera_config);
    }
  }
...
void BevTracker::PredictPoseByOdom() {
  ...
  current_frame_.trans_world2base_ =
      last_frame_.trans_world2base_ * odom_increment;

  double trajectory_increment =
      hypot(odom_increment(0, 2), odom_increment(1, 2));
...
bool BevTracker::NeedKeyFrame() {
  ...
  std::vector<KeyFrame *> connected_keyframes =
      ref_keyfm_->GetConnectedKeyframes();
  ...
  double dist = hypot(current_frame_.trans_world2base_(0, 2) -
                          ref_keyfm_->trans_world2base_(0, 2),
                      current_frame_.trans_world2base_(1, 2) -
                          ref_keyfm_->trans_world2base_(1, 2));
...
void BevTracker::CreateMapMappoints() {
  ...
  const unsigned int map_prob_min_slot =
      SystemConfig::GetSystemConfig()->map_prob_min_slot_;
  const unsigned int map_prob_min_dash_ =
      SystemConfig::GetSystemConfig()->map_prob_min_dash_;
  const unsigned int map_prob_min_arrow =
      SystemConfig::GetSystemConfig()->map_prob_min_arrow_;
  const unsigned int map_prob_min_lane =
      SystemConfig::GetSystemConfig()->map_prob_min_dash_;
  ...
  timer.Print();
  PlotViewer::GetInstance().UpdateData("create-map-points",
...
bool BevTracker::RemapImageRequired() {
  return trajectory_length_ <
         SystemConfig::GetSystemConfig()->feature_map_length_;
}
...
void BevTracker::GenerateEnergyMat(std::vector<unsigned char *> imgs) {
  ...
  avp_distance_transformer_->DistranceTransform(
      imgs[0], current_frame_.image_slot_.data);
  avp_distance_transformer_->DistranceTransform(
      imgs[1], current_frame_.image_dash_.data);
  avp_distance_transformer_->DistranceTransform(
      imgs[2], current_frame_.image_arrow_.data);
  avp_distance_transformer_->DistranceTransform(
      imgs[3], current_frame_.image_lane_.data);

  timer.Print();

#ifdef ENABLE_VIEWER
  PlotViewer::GetInstance().UpdateData("distance-transform",
                                       timer.GetTimeConsuming());
#endif
}
}
