  ...
  for (auto ngh_keyfrm : cur_covisibility_keyframes) {
    ...
    for (int ikp = 0; ikp < num_of_matches; ikp++) {
      ...
      if (cos_rays_parallax > 0 &&
        ...
        pos_w = Triangulator::Triangulate(ray_c_1, ray_c_2, cam_pose_1w_,
                                          cam_pose_2w_);
      ...
      float reproj_err_norm_cur =
          hypot(reproj_in_cur.x - keypt1.pt.x, reproj_in_cur.y - keypt1.pt.y);
      float reproj_err_norm_ngh =
          hypot(reproj_in_ngh.x - keypt2.pt.x, reproj_in_ngh.y - keypt2.pt.y);

      // checkout reprojection  error

      if (reproj_err_norm_cur * reproj_err_norm_cur >
          chi_sq_2D * cur_keyfrm->level_sigma2_[keypt1.octave])
        continue;
      ...
      cv::Mat pos_w3d =
          (cv::Mat_<float>(3, 1) << pos_w.x(), pos_w.y(), pos_w.z());
...
void LocalMapping::SearchFrameInNeighbors(KeyFrame *cur_keyfrm) {
  ...
  fuse_condidate_mappoints.reserve(fuse_tgt_keyfrms.size() *
                                   mappoint_matches.size());

  for (auto fuse_tgt_keyfrm : fuse_tgt_keyfrms) {

    vector<MapPoint *> mappoint_matches_tgt =
        fuse_tgt_keyfrm->GetMapPointMatches();
