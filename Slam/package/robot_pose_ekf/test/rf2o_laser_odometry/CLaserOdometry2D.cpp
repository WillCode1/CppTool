...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  ...
  // 9. deal with lidar degradation, like long corridor, open space. 
  dealWithLidarDegradation();
  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    if (i == 0)
    {
      for (unsigned int u = 0; u < cols_i; u++)
      {
        ...
        if ((u>1)&&(u<cols_i-2))
        {
          if (std::isfinite(dcenter) && dcenter > 0.f)//if (dcenter > 0.f)
          {
            float sum = 0.f;
            float weight = 0.f;
...
void CLaserOdometry2D::dealWithLidarDegradation()
{
  static int total = 0;
  static int cnt = 0;
  // For the problem of lidar degradation, check the degree of freedom of information matrix. 
  // If 'min_coeff' is less than the threshold, the system will degenerate, and set the maximum covariance.
  if (Aw.size() > 0)
  {
    ++total;

    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> eigensolver(Aw.transpose() * Aw);
    auto min_coeff = eigensolver.eigenvalues().minCoeff();
    if (min_coeff < min_coeff_threshold)
    {
      cov_odo(0, 0) = cov_odo(1, 1) = cov_odo(2, 2) = 10000;

      drop_lo_times_per = ++cnt * 100.0 / total;
      // ROS_INFO("drop lo (%d/%d) times! percent %f%%", cnt, total, drop_lo_times_per);
    }

    ROS_INFO_STREAM_COND(false, "AtA eigenvalues minCoeff: " << min_coeff);
    ROS_INFO_COND(false, "AtA2 eigenvalues minCoeff: (%f, %f, %f)",
