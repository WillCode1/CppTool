...
#include "rf2o_laser_odometry/track.h"
...
public:
  ...
  ros::Subscriber laser_sub, initPose_sub, order_sub;
  ros::Publisher odom_pub;
  ros::ServiceServer track_service;

  bool scan_available();
  void handleLaser(const sensor_msgs::LaserScan& new_scan);
  ...
  bool trackService(rf2o_laser_odometry::track::Request& req,
                    rf2o_laser_odometry::track::Response& rsp);
  void ProcessCallBack(const std_msgs::Bool::ConstPtr& data);
};
  ...
  CLaserOdometry2D()
{
  ...
  pn.param<double>("min_coeff_threshold", min_coeff_threshold, 0.7);
  ...
  track_service = pn.advertiseService("track_odom", &CLaserOdometry2DNode::trackService, this);

  order_sub = n.subscribe<std_msgs::Bool>("/ct_replay/one_task_begin", 1, &CLaserOdometry2DNode::ProcessCallBack, this);
...
void CLaserOdometry2DNode::ProcessCallBack(const std_msgs::Bool::ConstPtr& data)
{
  first_laser_scan   = true;
  ROS_INFO("recv new bag analysis order");
//    if (data->data) {
//        this->processing = true;
//        ROS_INFO("recv order: start");
//    } else {
//        this->processing = false;
//        ROS_INFO("recv order: stop");
//    }
...
bool CLaserOdometry2DNode::setLaserPoseFromTf()
{
  ...
  try
  {
    ...
    ROS_INFO("Found tf between [%s] and [%s]",
             base_frame_id.c_str(), last_scan.header.frame_id.c_str());
    retrieved = true;
  }
  catch (tf::TransformException &ex)
  {
    ROS_ERROR("[%s] %s",last_scan.header.frame_id.c_str(), ex.what());
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::LaserScan::ConstPtr& new_scan)
{
    if (this->processing)
    {
        handleLaser(*new_scan);
    }
}

void CLaserOdometry2DNode::handleLaser(const sensor_msgs::LaserScan & new_scan)
{
  if (GT_pose_initialized)
  {
    //Keep in memory the last received laser_scan
    if(new_scan.angle_max-new_scan.angle_min > 4.5){
      int margin = (new_scan.angle_max-new_scan.angle_min - 4.5)/new_scan.angle_increment/2;
      int size = new_scan.ranges.size() - 2*margin;
      last_scan.header = new_scan.header;
      last_scan.angle_increment = new_scan.angle_increment;
      last_scan.angle_min = new_scan.angle_min + last_scan.angle_increment*margin;
      last_scan.angle_max = last_scan.angle_min + size*last_scan.angle_increment;
      last_scan.ranges.resize(size);
      for(int i = 0; i < size; i++)
        last_scan.ranges[i] = new_scan.ranges[i+margin];
    }else{
      last_scan = new_scan;
    }
    ...
    if (!first_laser_scan)
    {
      ...
      for (unsigned int i = 0; i<width; i++)
        range_wf(width - i - 1) = last_scan.ranges[i];
      new_scan_available = true;
    }
    else
    {
      ROS_INFO("recv first laser scan");
      initial_robot_pose.pose.pose.position.x = 0;
      initial_robot_pose.pose.pose.position.y = 0;
      initial_robot_pose.pose.pose.position.z = 0;
      initial_robot_pose.pose.pose.orientation.w = 1;
...
#if 1
  //set the covariance
  odom.pose.covariance[6 * 0 + 0] = cov_odo(0, 0);
  odom.pose.covariance[6 * 1 + 1] = cov_odo(1, 1);
  odom.pose.covariance[6 * 2 + 2] = 10000;
  odom.pose.covariance[6 * 3 + 3] = 10000;
  odom.pose.covariance[6 * 4 + 4] = 10000;
  odom.pose.covariance[6 * 5 + 5] = cov_odo(2, 2);
  ROS_INFO_COND(false, "rf2o pose cov: %f %f %f", cov_odo(0, 0), cov_odo(1, 1), cov_odo(2, 2));
  ROS_INFO_COND(false, "rf2o linear: %f", lin_speed);
#endif
  //publish the message
  odom_pub.publish(odom);
}

bool CLaserOdometry2DNode::trackService(rf2o_laser_odometry::track::Request& req,
                                        rf2o_laser_odometry::track::Response& rsp)
{
  handleLaser(req.scan);
