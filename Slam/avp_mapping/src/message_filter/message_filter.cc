...
MessageFilter::MessageFilter(const float gps_frequency,
                             const float odom_frequency)
...
void MessageFilter::AddOdomData(uint64_t microsecond,
...
bool MessageFilter::GetOdomByTimestamp(const uint64_t &timestamp,
...
bool MessageFilter::GetGpsByTimestamp(const uint64_t &timestamp,
                                      Eigen::Vector2d &gps) {
  bool found = false;
