...
ORBextractor::ORBextractor(int _nfeatures, float _scaleFactor, int _nlevels,
  ...
  mask_ = cv::imread(feature_mask, cv::IMREAD_GRAYSCALE);
  if (mask_.empty()) {
    std::cout << " mask " << feature_mask << " is not found " << std::endl;
    mask_ = cv::Mat::zeros(kImgWidth, kImgWidth, CV_8UC1);
  }

  cv::resize(mask_, mask_, cv::Size(kImgWidth, kImgWidth));
  mask_ = mask_.rowRange(kImgLowBound, kImgUpBound);
...
bool ORBextractor::Inborder(cv::Point2f kp, int level) {
  float scale = mvScaleFactor[level];

  Point2f pt = kp * scale;
  uchar value = mask_.at<uchar>(pt.y, pt.x);
  if (value < 200)
    return false;
  return true;
}
...
void ORBextractor::operator()(InputArray _image, InputArray _mask,
  ...
  for (int level = 0; level < nlevels; ++level) {
    ...
    for (; keypoint != keypointEnd;) {
      if (Inborder(keypoint->pt, level)) {
        ++keypoint;
        continue;
      } else {
