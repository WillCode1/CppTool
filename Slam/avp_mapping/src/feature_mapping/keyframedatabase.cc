...
KeyFrameDatabase::KeyFrameDatabase(const ORBVocabulary &voc) : mpVoc(&voc) {
  ...
  //    mvInvertedMCFile.reserve(voc.size());
}
...
void KeyFrameDatabase::add(KeyFrame *pKF) {
  ...
  // 为每一个word添加该KeyFrame
  for (DBoW2::BowVector::const_iterator vit = pKF->bow_vec_.begin(),
                                        vend = pKF->bow_vec_.end();
       vit != vend; vit++)
    mvInvertedFile[vit->first].push_back(pKF);
}

// void KeyFrameDatabase::addMC(MCKeyFrame *pMCKF)
//{

//   unique_lock<mutex> lock(mMutex);

//   if(mspMCKFs.count(pMCKF))
//       return ;
//    for(int camId = 0 ; camId<4; camId++)
//    {
//        KeyFrame* pKF  = pMCKF->mvpKfs[camId];
//        for(DBoW2::BowVector::const_iterator vit= pKF->mBowVec.begin(),
//        vend=pKF->mBowVec.end(); vit!=vend; vit++)
//            mvInvertedMCFile[vit->first].push_back(pMCKF);
//    }
//    mspMCKFs.insert(pMCKF);
//}
  ...
  // 步骤1：找出和当前帧具有公共单词的所有关键帧（不包括与当前帧链接的关键帧）
  {
    ...
    for (DBoW2::BowVector::const_iterator vit = pKF->bow_vec_.begin(),
      ...
      for (list<KeyFrame *>::iterator lit = lKFs.begin(), lend = lKFs.end();
           lit != lend; lit++) {
        KeyFrame *pKFi = *lit;
        if (pKFi->mnLoopQuery != pKF->id_) // pKFi还没有标记为pKF的候选帧
        {
          pKFi->mnLoopWords = 0;
          ...
          {
            pKFi->mnLoopQuery =
                pKF->id_; // pKFi标记为pKF的候选帧，之后直接跳过判断
...
vector<KeyFrame *> KeyFrameDatabase::DetectMapFusionCandidates(KeyFrame *pKF) {
  list<KeyFrame *> lKFsSharingWords; // 用于保存可能与F形成回环的候选帧（只要有相同的word，且不属于局部相连帧）
  {
    ...
    for (DBoW2::BowVector::const_iterator vit = pKF->bow_vec_.begin(),
      ...
      for (list<KeyFrame *>::iterator lit = lKFs.begin(), lend = lKFs.end();
           lit != lend; lit++) {
        KeyFrame *pKFi = *lit;
        if (pKFi->mnRelocQuery != pKF->id_) // pKFi还没有标记为pKF的候选帧
        {
          pKFi->mnRelocWords = 0;
          pKFi->mnRelocQuery = pKF->id_;
          lKFsSharingWords.push_back(pKFi);
        }
        pKFi->mnRelocWords++;
      }
...
vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F) {
  ...
  // 步骤1：找出和当前帧具有公共单词的所有关键帧
  {
    ...
    // words是检测图像是否匹配的枢纽，遍历该pKF的每一个word
    for (DBoW2::BowVector::const_iterator vit = F->bow_vec_.begin(),
                                          vend = F->bow_vec_.end();
         vit != vend; vit++) {
      // 提取所有包含该word的KeyFrame
      list<KeyFrame *> &lKFs = mvInvertedFile[vit->first];
      ...
      for (list<KeyFrame *>::iterator lit = lKFs.begin(), lend = lKFs.end();
           lit != lend; lit++) {
        KeyFrame *pKFi = *lit;
        if (pKFi->mnRelocQuery != F->id_) // pKFi还没有标记为pKF的候选帧
        {
          pKFi->mnRelocWords = 0;
  ...
  for (list<KeyFrame *>::iterator lit = lKFsSharingWords.begin(),
    ...
    if (pKFi->mnRelocWords > minCommonWords) {
      nscores++; // 这个变量后面没有用到
      float si = mpVoc->score(F->bow_vec_, pKFi->bow_vec_);
      pKFi->mRelocScore = si;
      lScoreAndMatch.push_back(make_pair(si, pKFi));
    }
  }
