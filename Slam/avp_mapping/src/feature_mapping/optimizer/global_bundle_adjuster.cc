...
GlobalBundleAdjuster::GlobalBundleAdjuster(Map *map, unsigned int iterations,
                                           MultiCam *multicam)
    : map_(map), num_iter_(iterations), multicam_(multicam) {}

void GlobalBundleAdjuster::Optimize() const {
  ...
  long unsigned int max_keyfrm_id = 0;
  ...
  for (auto mckeyfrm : mckeyfrms) {
    ...
    if (mckeyfrm->id_ > max_keyfrm_id) {
      max_keyfrm_id = mckeyfrm->id_;
    }
  }

  MultiCam *multi_cam = multicam_;

  const unsigned long int kMaxPoseId = max_keyfrm_id;

  //  const float thHuber2D = sqrt(3.99);
  constexpr float chi_sqr_2d = 5.99146;

  std::vector<g2o::EquirectangularEdge *> edges;
  for (size_t i = 0; i < mps.size(); i++) {
    ...
    const int id = mp->id_ + max_keyfrm_id + 1;
