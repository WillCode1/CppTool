...
void Timer::Print()
{
  ...
  double time_duration = std::chrono::duration_cast<std::chrono::duration<double>>(end_time - start_timer_).count();
  ms_ = time_duration * 1000.0f;
  std::cout << " Timer(" << timer_name_ << ") : " << time_duration * 1000.0f << " ms " << std::endl;
}
