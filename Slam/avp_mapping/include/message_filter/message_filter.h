...
public:
  ...
  ~MessageFilter();
  void AddOdomData(uint64_t microsecond, const SemanticSLAM::WheelOdometry &odom);
  void AddGPS(uint64_t microsecond, const Eigen::Vector2d &gps);
  bool GetOdomByTimestamp(const uint64_t &timestamp, SemanticSLAM::WheelOdometry &odom);
