...
private:
  ...
  void Split(Node *node, const Box &box) {
    ...
    Eigen::Vector2f box_size = box.getSize();
    auto child_box_size =
        Eigen::Vector2f(box_size.x() / 2.0f, box_size.y() / 2.0f);
    node->children[0]->box = Box(node_box_origin, child_box_size);
    node->children[1]->box =
        Box(Eigen::Vector2f(node_box_origin.x() + child_box_size.x(),
                            node_box_origin.y()),
            child_box_size);
    node->children[2]->box =
        Box(Eigen::Vector2f(node_box_origin.x(),
                            node_box_origin.y() + child_box_size.y()),
            child_box_size);
    node->children[3]->box =
        Box(node_box_origin + child_box_size, child_box_size);
  }
};
}
