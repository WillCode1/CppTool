...
namespace SemanticSLAM
{
  ...
  public:
    ...
    virtual AVPPose GrabSegImages(uint64 microsecond, std::vector<unsigned char *> seg_imgs) = 0;
...
namespace FeatureSLAM
{
  ...
  public:
    ...
    virtual void InsertOdometry(uint64_t timestamp, const SemanticSLAM::WheelOdometry &odometry) = 0;

    virtual void AlignMaptoTrajectory(const std::string &file_name) = 0;
    virtual void Shutdown() = 0;
  };

  std::shared_ptr<FeatureMapper> CreateFeatureMapper(const std::string &config_file);
}
