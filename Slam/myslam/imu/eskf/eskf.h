...
namespace aw
{
    ...
    public:
        ...
        void imu_callback(const ImuDataPtr imu_msg)
        {
            ImuDataPtr imu_data_ptr = std::make_shared<ImuData>();
            imu_data_ptr->timestamp = imu_msg->timestamp;

            Eigen::Vector3d gyr(imu_msg->gyr[0], imu_msg->gyr[1], imu_msg->gyr[2]);
            Eigen::Vector3d acc(imu_msg->acc[0], imu_msg->acc[1], imu_msg->acc[2]);
        ...
        void predict(ImuDataConstPtr last_imu, ImuDataConstPtr curr_imu)
        {
            ...
            Fx.block<3, 3>(3, 6) = -state_ptr_->r_GI * aw::skew_matrix(acc_unbias) * dt;
        ...
        bool init_from_imudata(const Eigen::Matrix3d &rot, const Eigen::Vector3d &p)
        {
